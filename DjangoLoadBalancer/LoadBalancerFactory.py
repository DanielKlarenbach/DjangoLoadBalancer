...
class LoadBalancerFactory:
    ...
    def _create_databases(cls,result):
        if LOAD_BALANCER['CUD_ALGORITHM']=="MULTITHREADING":
            ...
            for i, name in enumerate(LOAD_BALANCER['DATABASES']):
                database = Database(name, {Wait.WAIT: WaitWaiter(LOAD_BALANCER['WAIT_TIME']), Wait.DONT_WAIT: DontWaitWaiter()},
                                    {QueryType.QUERYSET: QuerySetExecutor(result),QueryType.NO_QUERYSET: NoQuerySetExecutor(result)}, DATABASES[name]['ENGINE'],
    ...
    def _create_r_algorithm(databases):
        if LOAD_BALANCER['R_ALGORITHM']['NAME']=="ROUND_ROBIN":
            ...
            threading.Thread(target=algorithm.update_info, args=(LOAD_BALANCER['R_ALGORITHM']['INTERVAL'],QueryType.INFO_RESPONSE_TIME), daemon=True).start()
            return algorithm
            ...
            threading.Thread(target=algorithm.update_info, args=(LOAD_BALANCER['R_ALGORITHM']['INTERVAL'],QueryType.INFO_NUMBER_OF_CONNECTIONS), daemon=True).start()
            return algorithm
